...
import { FileText, Eye, EyeOff, Plus, Minus, Edit } from "lucide-react";
...
const EditMenu = () => {
  ...
  // Load edit data when menuEdit changes
  useEffect(() => {
    if (menuEdit) {
      setFormData({
        name: menuEdit.name,
        description: menuEdit.description,
        parentId: menuEdit.parentId || 0, // Handle null parentId
        displayOrder: menuEdit.displayOrder,
        level: menuEdit.level,
        isVisible: menuEdit.isVisible,
      });
      // Load children if any
      if (menuEdit.children && menuEdit.children.length > 0) {
        const childData: ChildCategory[] = menuEdit.children.map(
          (child: any) => ({
            id: child.id,
            name: child.name,
            slug: child.slug,
            description: child.description,
            parentId: child.parentId || 0, // Handle null parentId
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm() || !menuEdit) return;
    ...
    try {
      ...
      handleEditMenuAction(menuEdit.id, updatedMenu);
    } catch (error) {
          ...
          <div className="space-y-3">
            ...
            <Select
              options={[
                ...
                ...menu
                  .filter((item) => item.id !== menuEdit.id) // Exclude current item from parent options
                  .map((item) => ({
                    value: item.id,
                    label: item.name,
                  })),
            ...
            {/* Thông tin cấp độ */}
            <div className="p-3 bg-gradient-to-r rounded-lg border from-secondary/10 to-accent/10 border-border-secondary">
              <div className="flex items-center text-sm text-text-secondary">
                <div className="mr-2 w-2 h-2 bg-gradient-to-r rounded-full from-secondary to-accent"></div>
                      ...
                      <div className="mt-4 space-y-3">
                        ...
                        {child.children.length === 0 ? (
                          <div className="py-4 text-center text-text-muted">
                            <p className="text-sm">
                              Chưa có danh mục con (cấp 2) nào
                            </p>
                          </div>
                        ) : (
                          <div className="space-y-3">
          ...
          <div className="relative h-6">
            <div className="flex fixed right-0 bottom-0 left-0 justify-end p-2 space-x-3 rounded-b-lg text-text-primary bg-background-elevated">
              ...
              <Button
                ...
                disabled={loading}
                iconLeft={loading ? undefined : <Edit size={16} />}
              >
